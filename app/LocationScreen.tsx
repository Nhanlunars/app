  ...
  const getChargerByLocation = async () => {
    try {
      const response = await axios.get(
        `/api/get-all-charger-by-locationid?location_id=${"1"}`
      );
      setChargers(response.data.chargers.reverse()); // điều chỉnh nếu data nằm trong `response.data.data`
              ...
              <View key={charger.id} style={styles.stationCard}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  <Text style={styles.stationTitle}>
                    Tên Trạm: {charger.charger_name || "Không rõ"}
                  </Text>
                  <Text style={styles.stationSubtitle}>
                    Địa Chỉ:" " || "Chưa có vị trí"
                  </Text>
                </View>
                <TouchableOpacity>
